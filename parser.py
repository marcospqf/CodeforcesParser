...
language_params = {
        'c++14' : {
            'TEMPLATE'    : '~/Documents/Codeforces/temp.cpp',
            'DEBUG_FLAGS' : '-DDEBUG',
...
def generate_test_script(folder, language, num_tests, problem):
    ...
    with open(folder + 'test.sh', 'w') as test:
        ...
        test.write(
            ...
            '    {run_cmd} < $INPUT_NAME$test_case > $MY_NAME$test_case\n'
            '    echo ========================================\n'
            ...
            '    echo ========================================\n'
            'done\n'
            .format(num_tests, run_cmd=param["RUN_CMD"]))
    os.system('chmod +x ' + folder + 'test.sh')


# Main function.
def parse(contest, language='c++14'):
    ...
    for index, problem in enumerate(content.problems):
        ...
        os.system('cp '+ TEMPLATE + ' ' + folder + problem +'.'+ TEMPLATE.split('.')[1])
